...
def checkKeys(keyA, keyB, mode):
	...
	if keyA < 0 or keyB < 0 or keyB > len(SYMBOLS) - 1:
		sys.exit("Key A must be greater than 0 and key B must be between 0 ans %s." % (len(SYMBOLS) - 1))
...
def getRandomKey():
	while True:
		keyA = random.radint(2, len(SYMBOLS))
		keyB = random.randint(2, len(SYMBOLS))
		if cryptomath.gcd(keyA, len(SYMBOLS)) == 1:
			return keyA * len(SYMBOLS) + keyB

# If affineCipher.py is run (instead of imported as modulw), call
# the main() function:
